...
if ps_drush_bootstrapped && [[ "${FORCE_INSTALL}" != "1" ]]; then
  ...
  ps_timed_run "Prepare settings.php" ps_docker_exec_php "if [[ ! -f web/sites/default/settings.php ]]; then cp web/sites/default/default.settings.php web/sites/default/settings.php; fi && mkdir -p web/sites/default/files && chmod 664 web/sites/default/settings.php && chmod 775 web/sites/default/files"
...
ps_timed_run "Enable base contrib modules" ps_retry 2 2 ps_drush en -y token pathauto scheduler language locale content_translation config_translation \
  ...
  honeypot seckit \
  ui_patterns ui_styles

ps_timed_run "Enable front theme" ps_drush theme:enable -y ui_suite_bnp || true
ps_timed_run "Set front theme" ps_drush cset -y system.theme default ui_suite_bnp || true
...
ps_timed_run "Grant ps_favorite permission to ps_admin" ps_drush role:perm:add ps_admin "manage ps_favorite" -y || true
...
ps_timed_run "Enable anti-spam modules" ps_retry 2 2 ps_drush en -y captcha altcha
