...
const RULES = {
  ...
  colorNames: {
    ...
    message:
      'Color name instead of semantic variant (use primary, secondary, success, danger, etc.)',
    severity: 'error',
...
function printReport(fileIssues) {
  ...
  console.log(
    `Total issues: ${results.totalIssues} (${results.errors} errors, ${results.warnings} warnings)\n`
  );
