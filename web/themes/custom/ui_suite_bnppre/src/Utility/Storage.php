...
class Storage extends MemoryStorage implements \Iterator
{
  ...
  /**
   * Flag determining whether the cache should be saved to the database.
   *
   * @var bool
   */
   ...
   */
    public function __construct(string $cid, string $bin = 'default', int $expire = Cache::PERMANENT, array $tags = [Bootstrap::CACHE_TAG])
    {
      ...
      // Retrieve the cached data.
        $cached = $this->getCacheBackend()->get($this->cid);
  ...
  /**
   * Gets the cache backend for this storage bin.
   *
   ...
   * @internal
   *
   * @see \Drupal\ui_suite_bnppre\Utility\Storage::__construct
   */
    public function save(): void
    {
        if ($this->changed) {
            $this->getCacheBackend()->set($this->cid, $this->getAll(), $this->expire, $this->tags);
        }
    }
