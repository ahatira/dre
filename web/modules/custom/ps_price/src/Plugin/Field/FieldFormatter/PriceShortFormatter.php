...
/**
 * Short price field formatter.
 *
 * Displays price in short format (amount + currency only).
 */
#[FieldFormatter(
  id: 'ps_price_short',
  label: new TranslatableMarkup('Short'),
...
class PriceShortFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode): array {
    ...
    foreach ($items as $delta => $item) {
      $elements[$delta] = [
        '#markup' => $this->priceFormatter->formatShort($item, [
          'show_currency' => TRUE,
        ]),
      ];
    }
