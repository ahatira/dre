...
Représentation visuelle d'un utilisateur ou entité : photo, initiales, ou icône par défaut. Supporte plusieurs tailles (xs, sm, md, lg, xl), formes (circle, square, rounded), badges de statut (online, offline, busy), et états (clickable, bordered). Fallback automatique initiales → icône par défaut.
