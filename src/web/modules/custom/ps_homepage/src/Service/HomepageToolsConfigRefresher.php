...
/**
 * Refreshes the §3 tools section on the homepage layout from defaults.
 */
final class HomepageToolsConfigRefresher {

  private const BODY_PLUGIN_ID = 'ps_content_outils_accordion_block';
  ...
  /**
   * Applies default §3 tools configuration to the front page homepage.
   */
  ...
  private function buildRefreshedSections(NodeInterface $translation, string $langcode, array $defaults): ?array {
    ...
    foreach ($field->getSections() as $section) {
      ...
      if (!$this->sectionIsTools($section)) {
        $nextSections[] = $section;
        continue;
      }
  ...
  private function sectionIsTools(Section $section): bool {
    foreach ($section->getComponents() as $component) {
      if (!$component instanceof SectionComponent) {
        continue;
      }
