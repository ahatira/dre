...
final class ContactEmailHeroUploadHelper {
  ...
  /**
   * Form API #after_build callback: preview + focal point UI beside upload.
   *
   ...
   */
  public static function afterBuildWebformHeroUpload(array $element, FormStateInterface $form_state): array {
    /** @var self $helper */
    $helper = \Drupal::service('ps_form.contact_email_hero_upload_helper');
    return $helper->attachFocalPointUi($element, $form_state);
  }
  ...
  public function attachFocalPointUi(array $element, FormStateInterface $form_state): array {
    $file = $this->resolveUploadFile($element);
    if ($file === NULL) {
      return $element;
    }
  ...
  /**
   * Resolves the uploaded hero file from the hero_file container element.
   *
   * @param array<string, mixed> $element
   *   The hero_file container with a managed_file upload child.
   */
  private function resolveUploadFile(array &$element): ?FileInterface {
    ...
    if (!empty($upload['#files'])) {
      $file = reset($upload['#files']);
      return $file === FALSE ? NULL : $file;
    }

    $fid = ManagedFileFormValueHelper::extractManagedFileFid($upload['#default_value'] ?? NULL);
    if ($fid <= 0) {
      return NULL;
    }
