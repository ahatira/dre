...
/**
 * Admin UI tweaks for the search configuration hub.
 */
final class SearchAdminHooks {

  private const TRANSLATION_TASK = 'config_translation.local_tasks:config_translation.item.overview.ps_search.seo_url_mappings_form';
  ...
  private const ROUTE_TRANSLATION_TASK = [
    'ps_search.seo_url_mappings_form' => self::TRANSLATION_TASK,
    'config_translation.item.overview.ps_search.seo_url_mappings_form' => self::TRANSLATION_TASK,
  ];

  /**
   * Shows Translate on the primary tab row for SEO URL mappings.
   */
  ...
  public function localTasksAlter(array &$local_tasks): void {
    ...
    $local_tasks[self::TRANSLATION_TASK]['base_route'] = 'ps_search.admin_overview';
  ...
  /**
   * Renames the Translate tab label on the SEO URL mappings form.
   */
