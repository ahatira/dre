...
JSON_COUNT=$(curl -sL -m 60 "$BASE/ps-search/count?asset_type=BUR&operation_type=RENT" | python3 -c "import sys,json; print(json.load(sys.stdin).get('count', -1))" 2>/dev/null || echo "-1")
HTMX_BODY=$(curl -sL -m 60 -H "HX-Request: true" "$BASE/ps-search-filters/htmx/count-label?asset_type=BUR&operation_type=RENT" 2>/dev/null || echo "")
...
APPLY_HEADERS=$(curl -sI -m 60 -H "HX-Request: true" \
  "$BASE/ps-search-filters/htmx/apply-type?asset_type=BUR&operation_type=RENT" 2>/dev/null || echo "")
...
LOC_JSON_COUNT=$(curl -sL -m 60 "$BASE/ps-search/count?locality=Paris" | python3 -c "import sys,json; print(json.load(sys.stdin).get('count', -1))" 2>/dev/null || echo "-1")
LOC_HTMX_BODY=$(curl -sL -m 60 -H "HX-Request: true" "$BASE/ps-search-filters/htmx/count-label?locality=Paris" 2>/dev/null || echo "")
...
LOC_APPLY_HEADERS=$(curl -sI -m 60 -H "HX-Request: true" \
  "$BASE/ps-search-filters/htmx/apply-location?locality=Paris" 2>/dev/null || echo "")
...
SURF_JSON_COUNT=$(curl -sL -m 60 "$BASE/ps-search/count?surface_min=100" | python3 -c "import sys,json; print(json.load(sys.stdin).get('count', -1))" 2>/dev/null || echo "-1")
SURF_HTMX_BODY=$(curl -sL -m 60 -H "HX-Request: true" "$BASE/ps-search-filters/htmx/count-label?surface_min=100" 2>/dev/null || echo "")
...
SURF_APPLY_HEADERS=$(curl -sI -m 60 -H "HX-Request: true" \
  "$BASE/ps-search-filters/htmx/apply-range/surface?surface_min=100" 2>/dev/null || echo "")
...
MOBILE_APPLY_HEADERS=$(curl -sI -m 60 -H "HX-Request: true" \
  "$BASE/ps-search-filters/htmx/apply-mobile" 2>/dev/null || echo "")
...
HEADER_BODY=$(curl -sL -m 60 -H "HX-Request: true" \
  "$BASE/ps-search-filters/htmx/results-header?surface_min=100" 2>/dev/null || echo "")
...
if echo "$HEADER_BODY" | grep -q '424'; then
  pass "HTMX results-header reflects surface_min=100 count (424)"
else
  fail "HTMX results-header count not 424 for surface_min=100"
fi
