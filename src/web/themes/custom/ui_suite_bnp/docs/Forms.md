...
UI Suite BNP introduces two new properties (among others, see the
examples):
* `#input_group_after`
* `#input_group_before`
...
UI Suite BNP introduces a new property: `#floating_label`.
...
Also, if no placeholder attributes is set. UI Suite BNP will fall back to
the label itself.
