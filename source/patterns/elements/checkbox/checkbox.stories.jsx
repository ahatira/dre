...
/**
 * En Contexte: Real-world property search examples
 */
export const EnContexte = {
  name: 'In Context',
  render: () => `
    <div style="display: flex; flex-direction: column; gap: var(--size-7); max-width: 800px;">
      
      <!-- Property Type Filter (Vertical List) -->
      <div>
        <h3 style="margin: 0 0 var(--size-4) 0; font-size: var(--font-size-3); font-weight: 600; color: var(--gray-800);">Property Type</h3>
        <div style="display: flex; flex-direction: column; gap: var(--size-3);">
          ${checkboxTwig({ name: 'property-type', value: 'apartment', label: 'Apartment', checked: true })}
          ${checkboxTwig({ name: 'property-type', value: 'house', label: 'House', checked: false })}
          ${checkboxTwig({ name: 'property-type', value: 'commercial', label: 'Commercial Space', checked: false })}
          ${checkboxTwig({ name: 'property-type', value: 'land', label: 'Land', checked: false })}
        </div>
      </div>

      <!-- Amenities Grid (2 Columns) -->
      <div>
        <h3 style="margin: 0 0 var(--size-4) 0; font-size: var(--font-size-3); font-weight: 600; color: var(--gray-800);">Amenities</h3>
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--size-3);">
          ${checkboxTwig({ name: 'amenities', value: 'parking', label: 'Parking', checked: true })}
          ${checkboxTwig({ name: 'amenities', value: 'elevator', label: 'Elevator', checked: true })}
          ${checkboxTwig({ name: 'amenities', value: 'balcony', label: 'Balcony', checked: false })}
          ${checkboxTwig({ name: 'amenities', value: 'gym', label: 'Fitness Center', checked: false })}
          ${checkboxTwig({ name: 'amenities', value: 'pool', label: 'Swimming Pool', checked: false })}
          ${checkboxTwig({ name: 'amenities', value: 'terrace', label: 'Rooftop Terrace', checked: false })}
        </div>
      </div>

      <!-- Inline Agreement -->
      <div style="padding: var(--size-4); background-color: var(--gray-50); border-radius: var(--radius-2); border: 1px solid var(--border-light);">
        ${checkboxTwig({
          name: 'terms',
          value: 'accepted',
          label:
            'I agree to the terms and conditions of the real estate transaction, including payment schedule and property inspection requirements.',
          checked: false,
        })}
      </div>

      <!-- Card with Selection -->
      <div style="padding: var(--size-5); background-color: var(--white); border-radius: var(--radius-3); border: 1px solid var(--border-light); box-shadow: var(--shadow-1);">
        <div style="display: flex; gap: var(--size-4); align-items: start;">
          ${checkboxTwig({ name: 'property-select', value: '12345', label: null, checked: false })}
          <div style="flex: 1;">
            <h4 style="margin: 0 0 var(--size-2) 0; font-size: var(--font-size-3); font-weight: 600;">Modern Apartment in Paris 8th</h4>
            <p style="margin: 0; font-size: var(--font-size-1); color: var(--gray-600);">125 m² • 2 bedrooms • Parking • Balcony</p>
            <p style="margin: var(--size-2) 0 0 0; font-size: var(--font-size-4); font-weight: 700; color: var(--primary);">€850,000</p>
          </div>
        </div>
      </div>
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Real-world examples: property type filters (vertical), amenities grid (2 columns), terms agreement (long label), and property card with selection checkbox.',
      },
    },
  },
};
